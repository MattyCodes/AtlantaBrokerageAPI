...
RSpec.describe V1::FreeAtlantaMoveController, type: :controller do
  ...
  before :each do
    stub_const(
      'ApplicationController::CONTROLLER_DOMAIN_WHITELIST',
      TEST_CONTROLLER_DOMAIN_WHITELIST
    )
  end

  it 'Responds with a success JSON when the request host is valid.' do
    allow_any_instance_of(ActionController::TestRequest)
      .to receive(:referer).and_return('https://example.com')
  ...
  it 'Responds with a success JSON when the request host is valid.' do
    allow_any_instance_of(ActionController::TestRequest)
      .to receive(:referer).and_return('invalid.host')
